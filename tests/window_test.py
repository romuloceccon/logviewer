...
class BaseManagerTest(unittest.TestCase):
    class FakeManager(BaseManager):
        def __init__(self, curses, curses_window, chars):
    ...
    def test_should_create_manager_without_window(self):
        manager = BaseManagerTest.FakeManager(
            self._curses, self._curses_window, [])
    ...
    def test_should_refresh_one_window(self):
        manager = BaseManagerTest.FakeManager(
            self._curses, self._curses_window, [])
        manager.stack.append(self._window)
    ...
    def test_should_refresh_two_windows(self):
        ...
        manager.stack.append(second_window)
    ...
    def test_should_handle_generic_key(self):
        manager = BaseManagerTest.FakeManager(
            self._curses, self._curses_window, ['q'])
        manager.stack.append(self._window)

        manager.loop()
        self._window.handle_key.assert_called_once_with('q')
    ...
    def test_should_handle_key_with_two_windows(self):
        ...
        manager.loop()
        self.assertIsNone(self._window.handle_key.call_args)
        second_window.handle_key.assert_called_once_with('q')

    def test_should_handle_resize(self):
        manager = BaseManagerTest.FakeManager(
            self._curses, self._curses_window, [curses.KEY_RESIZE])
        manager.stack.append(self._window)
